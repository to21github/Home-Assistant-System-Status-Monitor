...
# ---- 采集缓存（避免并行请求重复采集）----
# 使用模块级变量 + 显式锁做双重检查缓存, 不再依赖 gunicorn worker 线程模型;
# 切换 gthread/gevent 等多线程 worker 时同样安全。
_cache = None
_cache_time = 0
_CACHE_TTL = 2  # 秒
...
_ha_collect_lock = threading.Lock()
...
def _collect_data_disk():
    ...
    try:
        ...
        _log.debug("statvfs(/data) 读取失败，回退本地挂载点", exc_info=True)
        return []
...
def _collect_disks():
    ...
    # 失败时回退本地挂载点。
    # 注意：不使用 Supervisor /host/info 的磁盘数据 —— 该 API 的磁盘数值单位
    # 随版本在字节/MiB/GiB 间变化，按字节解释会导致"磁盘严重不足"误报。
    return (
        _collect_data_disk()
...
def collect():
    """采集所有系统指标（带 2s 缓存，双重检查锁保护），返回字典"""
    now = time.time()
    if _cache is not None and (now - _cache_time) < _CACHE_TTL:
        return _cache

    with _collect_lock:
        # 双重检查: 拿到锁前缓存可能已被并发请求刷新
        now = time.time()
        if _cache is not None and (now - _cache_time) < _CACHE_TTL:
            return _cache
        try:
...
def _do_collect():
    ...
    # --- 磁盘（优先 statvfs 查 /data 数据分区，失败回退本地挂载点）---
